...
import { QuotationItem, QuotationStatus, Quotation, DiscountType } from '../../types';
import { TrashIcon } from '../../components/icons/TrashIcon';

type EditableQuotationItem = Omit<Partial<QuotationItem>, 'product'> & { productId?: string };

const EditQuotationPage: React.FC = () => {
  const { quotationId } = useParams<{ quotationId: string }>();
  const navigate = useNavigate();
  const { customers, products, taxes, quotations, updateQuotation } = useData();
  ...
  useEffect(() => {
    ...
    if (foundQuotation) {
      setQuotationToEdit(foundQuotation);
      setSelectedCustomerId(foundQuotation.customer.id);
      setIssueDate(foundQuotation.issueDate);
      setExpiryDate(foundQuotation.expiryDate);
      const editableItems = foundQuotation.items.map(item => ({
  ...
  const handleItemChange = (index: number, field: keyof EditableQuotationItem, value: any) => {
  ...
  const handleSave = (status: QuotationStatus) => {
    ...
    const finalItems: QuotationItem[] = items.map(item => {
    ...
    updateQuotation(quotationId, updatedQuotationData);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Edit Quotation #{quotationToEdit.quotationNumber}</h1>
        <div className="flex space-x-2">
            <Button variant="outline" onClick={() => handleSave(QuotationStatus.Draft)}>Save as Draft</Button>
            <Button onClick={() => handleSave(QuotationStatus.Sent)}>Update & Send</Button>
        </div>
      </div>
          ...
          <Card>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="text-right">
                    <h2 className="text-2xl font-bold uppercase text-gray-800 dark:text-white">Quotation</h2>
